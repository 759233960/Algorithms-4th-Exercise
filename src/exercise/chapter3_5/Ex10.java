...
public class Ex10<Key extends Comparable<Key>, Value> implements Iterable<Key> {
    ...
    public static void main(String[] args) {
        Ex09<Integer, String> st = new Ex09<>();
        st.put(1, "a");
        st.put(2, "b");
        st.put(3, "c");
        st.put(14, "y");
    ...
    private Node delete(Node h, Key key) {
        if (key.compareTo(h.key) < 0) {
            ...
            if (!isRed(h.right) && !isRed(h.right.left))
                h = rotateRight(h);
            if (key.compareTo(h.key) == 0) {
                h.values = get(h.right, min(h.right).key);
